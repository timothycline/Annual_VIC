...
#for cat in cats[0:1]:
    ...
    # Intersect catchment/grid with USFS lands (this is optional)
    arcpy.Intersect_analysis(in_features=catgrid + " #;" + forests + " #",
        out_feature_class=catgridfor,
        join_attributes="ALL", cluster_tolerance="-1 Unknown", output_type="INPUT")

    # Get percent of the grid/catchment intersection that is in USFS lands
    arcpy.AddField_management(in_table=catgridfor, field_name="ForPct", field_type="FLOAT",
                              field_precision="", field_scale="", field_length="", field_alias="",
                              field_is_nullable="NULLABLE", field_is_required="NON_REQUIRED", field_domain="")
    arcpy.CalculateField_management(in_table=catgridfor, field="ForPct",
                                    expression="!shape.area@hectares!/ !CatGridHa!", expression_type="PYTHON_9.3",
                                    code_block="")

    # Join this back to the catchment/grid intersection file
    arcpy.JoinField_management(in_data=catgrid, in_field="OBJECTID",
                               join_table=catgridfor, join_field="FID_" + cat + "_Grid",
                               fields="ForPct")
