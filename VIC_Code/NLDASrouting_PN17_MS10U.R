...
rn_names <- c("PN17",'MS10U')

#Read in all Runoff data as a list
AllRuns <- lapply(alldailyfiles[1:length(alldailyfiles)],FUN=function(x){
  RDS1<-readRDS(here('NLDASdata','DailyRunoffs',x))
  TOTRUN<-RDS1$BGRUN + RDS1$SSRUN
  return(TOTRUN)
})
names(AllRuns) <- alldates
startdate <- alldates[1]
enddate <- names(AllRuns)[length(AllRuns)]

TotalRunoffArray<-array(unlist(AllRuns),dim=c(224,464,length(alldailyfiles)))
...
# Get catchment areas, convert to sq meters
